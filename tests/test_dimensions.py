...
class QueryDistributionTest(unittest.TestCase):
    def test_no_drift_when_same_distribution(self):
        ...
        # Establish a baseline window of "normal" behavior — slightly
        # off-centroid but consistent.
        rng = random.Random(0)
        for _ in range(5):
            for _ in range(50):
        ...
        self.assertEqual(report.sample_size, 50)

    def test_flags_when_distribution_shifts(self):
        ...
        for _ in range(8):
            for _ in range(40):
                dim.update(
                    Trace(
                        timestamp=_ts(0),
                        query_embedding=[1.0 + rng.gauss(0, 0.001), rng.gauss(0, 0.001)],
                    )
                )
            dim.commit_window()
        ...
        for _ in range(40):
            dim.update(
                Trace(
                    timestamp=_ts(0),
                    query_embedding=[rng.gauss(0, 0.001), 1.0 + rng.gauss(0, 0.001)],
                )
            )
        report = dim.report()
        self.assertIs(report.severity, Severity.DEGRADED)
        self.assertIsNotNone(report.value)
        self.assertGreater(report.value, 0.5)

    def test_no_traces_returns_ok(self):
        dim = QueryDistribution()
        dim.set_reference([[1.0, 0.0]])
        self.assertIs(dim.report().severity, Severity.OK)


class RetrievalRelevanceTest(unittest.TestCase):
    def test_drop_in_hit_rate_alerts(self):
...
class JudgeDriftTest(unittest.TestCase):
    def test_flags_systematic_shift(self):
    ...
    def test_ignores_unknown_reference(self):
        dim = JudgeDrift(score_key="faithfulness")
...
class EmbeddingDriftTest(unittest.TestCase):
    def test_basic(self):
        dim = EmbeddingDrift()
